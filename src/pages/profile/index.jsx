...
export default function Profile() {
  ...
  const avatar = info.hoTen
    ? info.hoTen.charAt(0).toUpperCase()
    : info.taiKhoan
    ? info.taiKhoan.charAt(0).toUpperCase()
    : "U";
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded shadow mt-10">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <div className="text-2xl font-bold">{info.hoTen}</div>
          <div className="text-gray-500">@{info.taiKhoan}</div>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <span className="font-medium">Nhóm:</span> {info.maNhom || "GP01"}
          </div>
          <div>
          ...
          {info.ngayTao && (
            <div>
              ...
              {new Date(info.ngayTao).toLocaleDateString()}
            </div>
          )}
          {info.trangThai && (
            <div>
              <span className="font-medium">Trạng thái:</span> {info.trangThai}
            </div>
          )}
        </div>
        <button
